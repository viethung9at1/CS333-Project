...
void handle_SC_Open(){
	...
	if (freeSlot != -1) //Process when empty slot exists
	{
		...
		{
			if ((kernel->fileSystem->fileSlot[freeSlot] = new FileSlot(kernel->fileSystem->Open(filename, type, freeSlot))) != NULL){ //Sucessful
...
void handle_SC_Exec(){
	...
	int n = kernel -> pTable -> ExecUpdate(name);
...
void handle_SC_Wait(){
	...
	int result = kernel -> sTable -> Wait(name);
...
void handle_SC_Signal(){
	...
	int result = kernel -> sTable -> Signal(name);
...
void handle_SC_Join(){
	...
	int result = kernel -> pTable -> JoinUpdate(n);
...
void handle_SC_Exit(){
	int ec = kernel->machine->ReadRegister(4);
	kernel->machine->WriteRegister(2, kernel->pTable->ExitUpdate(ec));
...
void handle_SC_CreateSemaphore(){
	...
	int result = kernel -> sTable -> Create(name, sVal);
