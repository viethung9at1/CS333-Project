...
void Scheduler::HandleProcessCompletion(){
    ...
    toBeDestroyed=NULL;
    if(!readyList->IsEmpty()){
        kernel->currentThread=readyList->RemoveFront();
        kernel->currentThread->space->Lock1();
        kernel->currentThread->RestoreUserState();
        kernel->currentThread->Resume();
    } else {
        kernel->currentThread=NULL;
    }
    (void) kernel->interrupt->SetLevel(oldLevel); //restore interrupt
}