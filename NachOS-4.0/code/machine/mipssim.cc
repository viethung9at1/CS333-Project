...
void Machine::Run() {
  ...
  // printf("done set status\n");
  for (;;) {
    // printf("new loop\n");
    OneInstruction(instr);
    // printf("done one instruction\n");
    kernel->interrupt->OneTick();
    // printf("done one tick\n");
    if (singleStep && (runUntilTime <= kernel->stats->totalTicks)) {
      // printf("calling in if\n");
      Debugger();
      // printf("done debugger\n");
    }
  }
...
void Machine::OneInstruction(Instruction *instr) {
  ...
  if (!ReadMem(registers[PCReg], 4, &raw)) {
    // printf("something fails when read mem\n");
  ...
  // printf("I dont know what they do but it is finished\n");
}
